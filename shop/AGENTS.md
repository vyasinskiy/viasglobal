...
2. **Второй экран (Screen 2 — `FiestaCalendarSection.tsx`)**:
   - **Хронологический порядок событий**: События выводятся строго в порядке возрастания отдаленности во времени (начиная с активного события текущей недели `🔥 Esta semana` и далее в будущее `+1 sem`, `+2 sem`, `+3 sem`...).
