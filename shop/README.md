...
2. **Второй экран (`FiestaCalendarSection.tsx`)**:
   - **Хронологический порядок событий**: События выводятся строго по возрастанию отдаленности во времени (от актуального события текущей недели `🔥 Esta semana` и далее в будущее `+1 sem`, `+2 sem`, `+3 sem`...).
