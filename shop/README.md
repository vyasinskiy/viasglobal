...
- **Основной домен**: [https://viasglobal.es](https://viasglobal.es) (зеркала `www.viasglobal.es` и `shop.viasglobal.es` с автоматическим редиректом)
- **B2B Оптовый портал компании**: [https://b2b.viasglobal.es](https://b2b.viasglobal.es)
...
### Защита изображений от обратного поиска (Google Lens / TinEye / Яндекс.Картинки)
В интернет-магазине развернут двухуровневый комплекс защиты визуального контента:
1. **Серверная трансформация (`scripts/scraper/core/imageTransformer.ts`)**:
   - При переносе фотографий поставщика в наш CDN каждое изображение автоматически трансформируется через библиотеку `sharp`:
     - **Микроповорот (1.9°)** — сбивает матрицу дескрипторов ключевых точек алгоритмов SIFT и ORB.
     - **Автоматический кроп (3.5%)** — сдвигает геометрический центр объекта и отсекает углы поворота.
     - **Микрорамка-паспарту (14px, цвет `#f8fafc`)** — изменяет соотношение сторон и периметр изображения.
     - **Сдвиг гаммы и баланса белого (gamma 1.03, brightness 1.02, saturation 1.03)** — видоизменяет цветовую гистограмму, сохраняя естественный премиальный вид товара.
     - **Аналоговое микрозерно (Adversarial Grain)** — наложение незаметного высокочастотного шума через SVG-фильтр `fractalNoise` (4% прозрачности), ломающего глубокие сверточные эмбеддинги нейросетевых моделей поиска (Vision Transformer, CLIP, ResNet).
     - **Конвертация в современный формат WebP** с качеством 87 и неявным суффиксом `_opt.webp` (под видом стандартной web-оптимизации).
2. **Безопасное хранение оригиналов и мгновенный откат (Zero-Risk)**:
   - В таблице `products` созданы колонки:
     - `original_main_image` — прямая ссылка на чистое исходное фото без изменений.
     - `original_images` — массив ссылок на исходные изображения галереи.
   - В Supabase Storage в одном и том же бакете `products` сохраняются оба файла:
     - Чистый оригинал: `products/<id>/img_0_<hash>.webp`
     - Защищенный вариант: `products/<id>/img_0_<hash>_opt.webp`
   - Если потребуется в один клик вернуться к чистым оригиналам:
     ```bash
     npx tsx scripts/scraper/rollback_images.ts
     ```
   - Вернуть обратно защищенные `_opt.webp`:
     ```bash
     npx tsx scripts/scraper/rollback_images.ts --to-opt
     ```
3. **Фронтенд-защита от Google Объектива (Google Lens)**:
   - В карточках каталога (`ProductCard.tsx`) и на детальной странице товара (`products/[id]/page.tsx`) поверх фото размещен защитный прозрачный слой, перехватывающий контекстное меню (`onContextMenu={(e) => e.preventDefault()}`).
   - Атрибуты `draggable={false}` и `user-select: none` блокируют перетаскивание картинки мышью в поисковую строку Google Lens.
4. **Пакетная обработка существующих фото в базе (Backfill)**:
   - Для обновления уже сохраненных в Supabase товаров:
     ```bash
