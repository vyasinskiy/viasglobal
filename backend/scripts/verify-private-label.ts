...
async function verifyPrivateLabel() {
  try {
    ...
    if (isSingleSellerDominant || isBuyBoxDominant) {
      ...
      console.log(`❌ [РЕЗУЛЬТАТ] НЕТ ПОДТВЕРЖДЕНИЯ Private Label: Это похоже на оптовые продажи (WHOLESALE).`);
      console.log(`   Листинги распределены между несколькими крупными независимыми продавцами.`);
    }
    console.log(`------------------------------------------------------\n`);
