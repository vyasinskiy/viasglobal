...
1. **`AsinView`**:
   - Назначение: удобная выборка товаров ASIN с базовыми полями, ценой Buy Box и расчетной максимальной ценой закупки.
   - Поля:
     ...
     - `maxBuyPrice` (Float, максимальная цена оптовой закупки под 20% маржи)
