...
6. `DOMINANT_BUY_BOX_SELLER` - доля владения BuyBox у топового продавца за 90 дней (`buyBoxTopSeller90Days`, в БД хранится как `0.0..1.0`) `>= 0.90` (90%, листинг монополизирован). Функция автоматически приводит целый параметр (например, 90) к коэффициенту 0.90.
