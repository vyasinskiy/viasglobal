...
const TARGET_BRAND_INPUT = 'YCBTPS';
const TARGET_SELLER_INPUT = 'A1EAYNIKZNYETZ';

// Результаты и заметки анализа Private Label (данные о юрлице, VAT/IVA, удержании Buy Box)
const ANALYSIS_NOTES = `Подтвержденный Private Label. Продавец — китайская компания (xianhejunshangmaoyouxiangongsi, регистрация CN). Продавец является единственным на 99.1% листингов каталога бренда, полностью монополизируя продажи. Бренд-нейм представляет собой типичный акроним для ТМ из КНР. Оптовый рынок отсутствует.`;
