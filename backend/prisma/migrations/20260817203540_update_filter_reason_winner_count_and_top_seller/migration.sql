-- Обновление функций get_asin_filter_reason и check_probable_private_label:
-- 1. Добавление проверки на минимальное количество победителей BuyBox: buyBoxWinnerCount90Days >= 3 (критерий от 3 продавцов).
--    Если победителей меньше 3, ASIN отсеивается с причиной FEW_BUYBOX_WINNERS.
-- 2. Проверка на доминирование топового продавца за 90 дней: buyBoxTopSeller90Days >= 90%.
--    Если топовый продавец удерживал BuyBox 90%+ времени, ASIN отсеивается с причиной DOMINANT_BUY_BOX_SELLER.
-- 3. Удаление старой ошибочной логики парсинга рейтинга отзывов в процентах из скобок строки продавца.
...
-- Обновляем функцию check_probable_private_label с поддержкой нового аргумента
