-- Обновление функции check_probable_private_label для вызова базовой функции get_asin_filter_reason
...
BEGIN
    ...
    -- И базовая функция помечает этот ASIN как DOMINANT_BUY_BOX_SELLER
    SELECT count(*)
    ...
    WHERE a2."brandId" = p_brand_id 
      ...
      -- ВЫЗЫВАЕМ БАЗОВУЮ ФУНКЦИЮ ДЛЯ ПРОВЕРКИ
      AND public.get_asin_filter_reason(a2.id, p_dominant_threshold) = 'DOMINANT_BUY_BOX_SELLER';
