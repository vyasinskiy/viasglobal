-- AlterTable
ALTER TABLE "KeepaExport" ADD COLUMN     "brandId" INTEGER;

-- AddForeignKey
ALTER TABLE "KeepaExport" ADD CONSTRAINT "KeepaExport_brandId_fkey" FOREIGN KEY ("brandId") REFERENCES "Brand"("id") ON DELETE SET NULL ON UPDATE CASCADE;
...
-- ==============================================================================
-- Функция: get_asin_filter_reason (Обновлена с вызовом check_brand_seller_dominance)
-- ==============================================================================
...
BEGIN
    ...
    IF v_brand_id IS NOT NULL AND v_seller_id IS NOT NULL AND v_seller_id <> '' THEN
        ...
        -- 3.1. ПРОВЕРКА НА ДОМИНИРОВАНИЕ ПРОДАВЦА НА УРОВНЕ КАТАЛОГА БРЕНДА (Macro Dominance)
        -- Если целевой продавец удерживает >= 80% товаров всего каталога бренда (при наличии обеих выгрузок), отсеиваем товар
